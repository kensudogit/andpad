# All-in-one: Go API (internal 8081) + Next.js (PORT) for a single Railway service.
# Root Directory must be empty (repo root). Config: /railway.toml
#
# Named Dockerfile so Railway Metal builder finds it even when
# dockerfilePath in railway.toml is ignored.

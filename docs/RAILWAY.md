...
## 1b. GitHub Actions 経由（推奨・空アーカイブ回避）

GitHub 連携の自動ビルドが空コンテキストになる場合は、Actions から CLI でルートを送ります。

1. Railway → Account Settings → **Tokens** → トークン作成
2. GitHub リポジトリ → Settings → Secrets → `RAILWAY_TOKEN` を追加
3. （任意）Variables に `RAILWAY_SERVICE` = サービス名
4. `main` へ push → workflow **Railway Deploy** が `railway up --path-as-root .` を実行

Dashboard の Root Directory は引き続き **空** にしてください（ランタイムの作業ディレクトリにも影響します）。
...
| **Config file path** | `/railway.toml` または `/railway.json` |
| Builder | Dockerfile → `Dockerfile`（統合イメージ） |
...
| `railway.json` | 同上（JSON。UI が toml を無視する場合の保険） |
...
| `frontend/railway.service.example.toml` | Web のみの 2 サービス構成用（通常は未使用） |
| `backend/railway.service.example.toml` | API のみの 2 サービス構成用（通常は未使用） |
...
| `couldn't locate the dockerfile at path Dockerfile` | **ほぼ空のビルドコンテキスト**。ログに snapshot ~55KB が出る場合、**Root Directory が `scripts` になっている**可能性が高い（`scripts/` のサイズと一致）。`frontend`/`backend` なら別の Dockerfile が見つかる | ① Settings → Build → **Root Directory を完全に空** ② **Config = `/railway.toml`** ③ **Dockerfile Path を空** ④ Redeploy。回避: `railway up --path-as-root .` または GitHub Actions（`RAILWAY_TOKEN` 秘密情報） |
